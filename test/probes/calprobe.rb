...
class Cal
  ...
  def initialize
    ...
    # year
    # opt_c
    @start = 2361222#START[DEFAULT_START]
  end

  # def opt_j(flag=false) @opt_j = flag end
  # def opt_m(flag=false) @opt_m = flag end
  # def opt_t(flag=false) @opt_t = flag end
  # def year(flag=false) @year = flag end
  # def opt_c(arg=DEFAULT_START) @start = START[arg] end

  # def set_params
  #   @dw = 2#if @opt_j then 3 else 2 end
  #   @mw = (@dw + 1) * 7 - 1
  #   @mn = 3#if @opt_j then 2 else 3 end
  #   @tw = (@mw + 2) * @mn - 2
  #   @k  = 0#if @opt_m then 1 else 0 end
  #   @da = :mday#if @opt_j then :yday else :mday end
  # end
...
if __FILE__ == $0
  puts Cal.render true
  puts Cal.render false
end
